...
public class ManagePersonServlet extends HttpServlet {
	...
	СОСТОЯНИЕ ПО РАБОТЕ НАД ДАННЫМ ПРОЕКТОМ.
	
	1. К проекту подключен Maven, установлен и развернут сервер Tomcat 9.0.
		Достигнуто взаимодействие с ним.
	
	2. Установлен и успешно запущен сервер MySQL. Успешно развернута из дампа база данных 
	 	phonebook с помощью MySQL Workbench 8.0
	 	
	 3. Установлен и подключен MySQL JDBC Driver 8.0.13
	 
	 4. На данный момент не удается подключиться к базе данных на сервере MySQL. Причина на данный момент 
	 не установлена.
